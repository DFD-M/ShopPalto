...
$mysqli->query("INSERT INTO company(company_name) VALUES ('{$company}')");
...
echo '<h1>Запись успешно добавлена</h1>';
    ...
    $result = $mysqli->query("SELECT * FROM company"); // запрос на выборку
while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
{
    echo '<p>Запись id='.$row['company_id'].'. Текст: '.$row['company_name'].'</p>';// выводим данные
}
