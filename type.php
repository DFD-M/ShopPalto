...
$mysqli->query("INSERT INTO type(type_name) VALUES ('{$type}')");
...
echo '<h1>Запись успешно добавлена</h1>';
    ...
    $result = $mysqli->query("SELECT * FROM type"); // запрос на выборку
while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
{
    echo '<p>Запись id='.$row['type_id'].'. Текст: '.$row['type_name'].'</p>';// выводим данные
}
